<template>
  <div class="whole-SMS">
    ...
    <div class="add list">
      <el-button type="primary" style="margin-right: 16px;" @click="addWholeSMS('add')" class="addList">
        <i class="el-icon-circle-plus-outline icon-sign"></i>
        新建短信
      </el-button>
      <el-button @click="getRecords" class="delete" style="margin-left:6px;">
        <i class="menuicon iconfont icon-lishi" style="vertical-align: top;"></i>
        历史记录
      </el-button>
    </div>
    <m-table v-if="tableData.length > 0" :tableData="tableData" :columns="tableColumns" @pageChange="pageChange" :stripe="true" :page="pageInfo" :border="false" layout="total, prev, pager, next, jumper">
      <div slot="btn" slot-scope="{row}">
        <div>
          <!-- 查看 -->
          <el-tooltip class="item" effect="light" content="点击查看" placement="top">
            <el-button class="btn-img" type="text" @click="getItemObj(row)"><img src="@/assets/marketSMS/chakan.png" alt="" /></el-button>
          </el-tooltip>
          <!-- 通过 -->
          <el-tooltip class="item" effect="light" content="点击通过" placement="top" v-if="toggleAdoptState(row)">
            <el-button class="btn-img" type="text" @click="adoptOrRefuseOrdel('adopt', row)"><img src="@/assets/marketSMS/tongguo.png" alt="" /></el-button>
          </el-tooltip>
          <!-- 拒绝 -->
          <el-tooltip class="item" effect="light" content="点击拒绝" placement="top" v-if="toggleRefuse(row)">
            <el-button class="btn-img" type="text" @click="adoptOrRefuseOrdel('refuse', row)"><img src="@/assets/marketSMS/jujue.png" alt="" /></el-button>
          </el-tooltip>
          <!-- 修改 -->
          <el-tooltip class="item" effect="light" content="点击修改" placement="top" v-if="toggleEdit(row)">
          ...
          <!-- 删除 -->
          <el-tooltip class="item" effect="light" content="点击删除" placement="top">
    ...
    <!-- 弹框动作 -->
    <el-dialog :visible.sync="centerDialogVisible" :width="diaTypeInfo === '确认通过?' ? '371px' : '322px'" center :show-close="false" top="22vh" class="dia-whole dia-form">
      <span class="diaLog-span" style="text-align:center;">{{diaTypeInfo}}</span>
      <p class="diaLog-p" v-if="diaTypeInfo === '确认通过?'">确认后该信息将直接发送，并且不 可撤回，确认通过吗？</p>
      <span slot="footer" class="dialog-footer">
        <el-button @click="centerDialogVisible = false" class="cancelSumbitBtn" style="margin-right: 20px;">取 消</el-button>
        <el-button type="primary" @click="deteleWholeDialog" class="sumbitBtn">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 短信详情 -->
    <el-dialog title="短信详情" :visible.sync="dialogVisible" width="660px" top="22vh" class="dia-form">
      <m-form :formData="formInfoData" :columns="formInfoColums" />
      <span slot="footer">
        <el-button type="primary" @click="dialogVisible = false" class="sumbitBtn">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default class WholeSMS extends mixins(MixinTools) {
  ...
  getItemObj (obj: any) {
    ...
    getListById(`${baseUrl}/marketing/${obj.id}`).then((res: any) => {
      if (res) {
        this.formInfoData = {
          ...res
        }
  ...
  async adoptOrRefuseOrdel (type: string, item: any) {
    try {
