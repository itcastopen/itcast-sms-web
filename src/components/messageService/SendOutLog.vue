<template>
  <div class="SendOutLog">
    <div class="search">
      <m-form :formData="formData" labelPosition="left" :columns="formColumns"></m-form>
    </div>
    <m-table v-if="tableData.length > 0" :table-data="tableData" :columns="tableColumns" @pageChange="pageChange" :border="false" :page="pageInfo" :stripe="true" layout="total, prev, pager, next, jumper">
...
@Component
export default class SendOutLog extends Vue {
  static componentName = 'SendOutLog'
  ...
  getList () {
    ...
    this.$api.getLogSendPage(params).then(res => {
      if (res && res !== null) {
...
<style lang="scss" scoped>
  .SendOutLog {
    ...
    /deep/ .el-date-table td.selected span {
      background-color:  #0bb290!important;
    }
